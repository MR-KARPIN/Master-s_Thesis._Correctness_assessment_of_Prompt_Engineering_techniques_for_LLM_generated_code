def findWeightedMedian(n, edges, queries):
    ...
    # Step 2: Precompute distances and parent
    depth = [0] * n
    dist = [0] * n
    parent = [-1] * n
    ...
    def dfs(node, par, d):
        ...
        for neighbor, weight in tree[node]:
            if neighbor != par:
                ...
                dfs(neighbor, node, d + weight)

    dfs(0, -1, 0)

    # Step 3: Implement LCA using Binary Lifting
    LOG = 17  # Up to 2^17 > 100000, for safety with n <= 100000
    ...
    def find_weighted_median(uj, vj):
        lca_node = lca(uj, vj)
        total_weight = dist[uj] + dist[vj] - 2 * dist[lca_node]
        half_weight = total_weight / 2

        # Walk from uj to the LCA
        current_node = uj
        path_weight = 0
        while current_node != lca_node:
            for neighbor, weight in tree[current_node]:
                if neighbor == parent[current_node]:
                    continue
                if depth[neighbor] < depth[current_node]:
                    continue
                path_weight += weight
                if path_weight >= half_weight:
                    return neighbor
                current_node = neighbor
                break

        # Walk from vj to the LCA
        current_node = vj
        path_weight = 0
        while current_node != lca_node:
            for neighbor, weight in tree[current_node]:
    ...
    for uj, vj in queries:
        ans.append(find_weighted_median(uj, vj))

    return ans

def generated_function(n,edges,queries):
    return findWeightedMedian(n,edges,queries)
